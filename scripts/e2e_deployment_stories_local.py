...
def test_cicd_runner_inventory_fallback_does_not_skip_deploy() -> None:
    ...
    for required in (
        "Cancel older same-branch workflow runs",
        ...
        if required not in workflow:
            raise AssertionError(f"CI runner check must clear stale older runs before self-hosted build: {required}")
...
def test_cicd_clean_checkout_asset_contract() -> None:
    ...
    if "cargo check --profile devdeploy -p slopmud -p shard_01 --bins" in workflow:
        raise AssertionError("dev validation must not pay for a duplicate cargo check before build")
