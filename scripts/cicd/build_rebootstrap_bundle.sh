...
cp "${repo_root}/scripts/restore_wal_backup.sh" "$tmpdir/restore_wal_backup.sh"
cat >"$tmpdir/run.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/restore_onebox_stack.sh" "$@"
EOF
chmod 0755 "$tmpdir/run.sh" "$tmpdir/restore_onebox_stack.sh" "$tmpdir/tls_cache_ssm.sh" "$tmpdir/certbot_deploy_hook_slopmud.sh" "$tmpdir/restore_wal_backup.sh"
