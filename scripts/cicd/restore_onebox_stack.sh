...
- restores TLS certs from SSM cache before minting via certbot
...
if [[ -z "$from_s3" && ( -z "$bucket" || -z "$track" ) ]]; then
...
ensure_packages() {
  if command -v apt-get >/dev/null 2>&1; then
    ...
    apt-get update -y
    apt-get install -y awscli ca-certificates certbot python3-certbot-dns-route53 rsync curl openssl
  elif command -v dnf >/dev/null 2>&1; then
    dnf -y install awscli ca-certificates certbot python3-certbot-dns-route53 rsync curl openssl || {
      dnf -y install awscli ca-certificates certbot python3-pip rsync curl openssl
...
latest_s3_uri() {
  local latest_pointer="${track}/latest/artifact.tgz"
  local latest_key

  if aws s3api head-object --bucket "$bucket" --key "$latest_pointer" >/dev/null 2>&1; then
    printf 's3://%s/%s\n' "$bucket" "$latest_pointer"
    return 0
  fi
...
ensure_hook_binary() {
  ...
  install -m 0755 "${script_dir}/tls_cache_ssm.sh" /usr/local/bin/slopmud-tls-cache-ssm
...
ensure_cached_cert() {
  ...
  local privkey_ssm="$4"
  local min_valid_days="$5"

  if [[ -z "$fullchain_ssm" || -z "$privkey_ssm" ]]; then
    return 1
  fi

  /usr/local/bin/slopmud-tls-cache-ssm ensure \
    --cert "$cert_path" \
    --key "$key_path" \
    --fullchain-ssm "$fullchain_ssm" \
    --privkey-ssm "$privkey_ssm" \
    --min-valid-days "$min_valid_days" \
    --owner slopmud \
    --group slopmud \
    --file-mode 0640
}
...
issue_or_sync_cert_for_env() {
  ...
  local cert_path=""
  local key_path=""
  local cache_fullchain_ssm=""
  local cache_privkey_ssm=""
  local cache_min_valid_days="5"
  ...
  cert_path="${TLS_CERT:-${dst_dir}/fullchain.pem}"
  key_path="${TLS_KEY:-${dst_dir}/privkey.pem}"
  cache_fullchain_ssm="${TLS_CACHE_FULLCHAIN_SSM:-}"
  cache_privkey_ssm="${TLS_CACHE_PRIVKEY_SSM:-}"
  cache_min_valid_days="${TLS_CACHE_MIN_VALID_DAYS:-5}"
  ...
  if [[ -z "${issued_certs[$marker_key]:-}" ]]; then
    if ! ensure_cached_cert "$cert_path" "$key_path" "$cache_fullchain_ssm" "$cache_privkey_ssm" "$cache_min_valid_days"; then
      ...
      TLS_CACHE_FULLCHAIN_SSM="$cache_fullchain_ssm" \
      TLS_CACHE_PRIVKEY_SSM="$cache_privkey_ssm" \
      /usr/local/bin/slopmud-certbot-sync
      store_cached_cert "$cert_path" "$key_path" "$cache_fullchain_ssm" "$cache_privkey_ssm" || true
    fi
...
export TLS_CACHE_FULLCHAIN_SSM=${cache_fullchain_ssm}
export TLS_CACHE_PRIVKEY_SSM=${cache_privkey_ssm}
...
EOF
  ...
  append_unit_env "$unit_path" "SHARD_RAFT_LOG"
  append_unit_env "$unit_path" "SHARD_PLAYERS_PATH"
  append_unit_env "$unit_path" "SLOPMUD_WAL_BACKUP_ENABLED"
  append_unit_env "$unit_path" "SLOPMUD_WAL_BACKUP_DIR"
  append_unit_env "$unit_path" "SLOPMUD_WAL_BACKUP_INTERVAL_S"
  append_unit_env "$unit_path" "SLOPMUD_WAL_BACKUP_MAX_SEGMENT_BYTES"
  append_unit_env "$unit_path" "SLOPMUD_WAL_BACKUP_MAX_LOCAL_MANIFESTS"
...
EOF
  ...
  append_unit_env "$unit_path" "SLOPMUD_PLAYERS_PATH"
...
EOF
  ...
  if [[ -n "${OIDC_USERS_PATH:-}" && ! -e "${OIDC_USERS_PATH}" ]]; then
    install -d -o slopmud -g slopmud -m 0755 "$(dirname "${OIDC_USERS_PATH}")"
    printf '%s\n' '{"users":[]}' >"${OIDC_USERS_PATH}"
    chown slopmud:slopmud "${OIDC_USERS_PATH}"
    chmod 0644 "${OIDC_USERS_PATH}"
  fi
...
EOF
  ...
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_AUTH_URL_SSM"
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_TOKEN_URL_SSM"
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_USERINFO_URL_SSM"
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_CLIENT_ID_SSM"
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_CLIENT_SECRET_SSM"
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_REDIRECT_URI_SSM"
  append_unit_env "$unit_path" "SLOPMUD_OIDC_SSO_SCOPE_SSM"
